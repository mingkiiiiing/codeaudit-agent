...
1. 版本单源：``audit.__version__`` 为合法 semver（当前 0.6.0）；
...
def test_version_is_current_release() -> None:
    """W7 版本单源约定：audit.__version__ 即唯一版本来源（semver 合法性由上方用例保证）。"""
    import audit

    assert audit.__version__  # 非空即可，具体值随发布演进（当前 0.6.0）
...
def test_console_script_smoke(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    - A1 已合入 --version：返回/退出 0，且输出含当前版本号（argparse action="version"
    ...
    try:
        ...
        assert rc in (0, None)
        captured = capsys.readouterr()
        assert audit.__version__ in captured.out
