...
- AUDITS 终态清理（ISSUE-R1-6）：当前实现任务表无界增长，断言修复后行为，
  以 strict xfail 标注，A3 修复后应转 XPASS 并移除标记。
"""
...
def test_three_concurrent_audits_no_cross_talk(
    ...
    # 每个任务的事件流各自非空（emitter 与任务一一对应）
    for n, audit_id in audit_ids.items():
        assert server_app.AUDITS[audit_id]["events"], n
...
def test_terminal_audits_evicted_under_lru_cap(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, make_project) -> None:
    ...
    for i in range(cap):
        server_app.AUDITS[f"prefilled-{i}"] = {
            "status": "done",
            "report": None,
            "events": [],
            "error": None,
    ...
    assert len(server_app.AUDITS) <= cap  # 容量上限生效
    assert "prefilled-0" not in server_app.AUDITS  # 最旧终态项被淘汰
    assert server_app.AUDITS[audit_id]["status"] == "done"  # 新任务可查询
