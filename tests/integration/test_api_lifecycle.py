...
- W11 适配（最小）：任务表为 TaskStore——autouse fixture 为每个用例注入独立临时库
  （monkeypatch server_app._STORE，隔离用例且零默认工作区落盘）；后台任务在独立
  线程执行，TestClient 以上下文管理器持有跨请求存活的 portal；
...
def test_api_full_lifecycle_with_real_pipeline(
    ...
    # 上下文管理器：跨请求存活的 portal（W11 线程执行形态下后台任务跨请求可见）
