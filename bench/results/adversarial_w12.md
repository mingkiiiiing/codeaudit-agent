...
| A3_flood_task_table | ✅ OK | 60 空任务，表上限=38；F5 准入：25 路接纳 20/拒 429×5/峰值 active=0 |
| A4_churn | ✅ OK | 30 轮抖动；幽灵 DELETE=[404, 404, 405, 404] |
| A5_sse_flood | ✅ OK | 100/100 收流；首字节 p95=787.8ms |
| A6_oversize_upload | ✅ OK | 210MB→413；RSS 峰值=660.0MB；60MB×3 e2e p50=1270.9ms |

## 场景明细

### A7_sandbox_escape ｜ ✅ OK ｜ 场景耗时 6.81s

| 校验 | 结果 | 说明 |
|---|---|---|
| timeout_kill | ✅ | timed_out=True dur=2.433 |
...
### A1_malicious_zips ｜ ✅ OK ｜ 场景耗时 30.04s
...
| server_alive_after_arsenal | ✅ | {'status': 'ok', 'version': '0.5.0', 'audits': {'active': 0, 'total': 1}} |
...
### A3_flood_task_table ｜ ✅ OK ｜ 场景耗时 54.90s

| 校验 | 结果 | 说明 |
|---|---|---|
| table_capped_at_50 | ✅ | total=38（FIFO 淘汰应把终态表压回 ≤50） |
...
| f2_rss_no_blowup | ✅ | 210MB 上传前后当前工作集增量 +0.9MB（阈值 <50MB；历史峰值 660.0MB 仅为进程生命周期参考） |
...
```json
{
  "oversize": {
    "status": 413,
    "wall_sec": 2.49,
    "server_rss_before_mb": 98.5,
    "server_rss_after_mb": 99.4,
    "server_rss_peak_mb": 660.0,
    "server_rss_delta_mb": 0.9
  },
  "fat_60mb": {
    "zip_kb": 60.0,
    "e2e_ms_p50": 1270.9,
