...
| A3_flood_task_table | ✅ OK | 60 空任务，表上限=50；F5 准入：25 路接纳 25/拒 429×0/峰值 active=0 |
| A4_churn | ✅ OK | 30 轮抖动；幽灵 DELETE=[404, 404, 405, 404] |
| A5_sse_flood | ✅ OK | 100/100 收流；首字节 p95=602.8ms |
| A6_oversize_upload | ✅ OK | 210MB→413；RSS 峰值=659.5MB；60MB×3 e2e p50=949.2ms |

## 场景明细

### A7_sandbox_escape ｜ ✅ OK ｜ 场景耗时 4.46s

| 校验 | 结果 | 说明 |
|---|---|---|
| timeout_kill | ✅ | timed_out=True dur=2.02 |
...
### A1_malicious_zips ｜ ✅ OK ｜ 场景耗时 23.15s
...
### A3_flood_task_table ｜ ✅ OK ｜ 场景耗时 60.34s

| 校验 | 结果 | 说明 |
|---|---|---|
| table_capped_at_50 | ✅ | total=50（FIFO 淘汰应把终态表压回 ≤50） |
| f5_submitted_all_resolved | ✅ | 接纳 25 + 拒绝 0 != 25 |
| f5_no_5xx | ✅ | [200] |
| f5_active_bounded | ✅ | active（queued+running 口径）峰值=0，契约上界=20 |
| f5_accepted_all_terminal | ✅ | ['done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done', 'done'] |
| server_alive | ✅ | {'status': 'ok', 'version': '0.5.0', 'audits': {'active': 0, 'total': 0}} |

```json
{
  "flood_count": 60,
  "upload_wall_sec": 1.13,
  "terminals": {
    "done": 50,
    "deleted": 10
  },
...
| f2_rss_no_blowup | ✅ | 210MB 上传前后当前工作集增量 +2.6MB（阈值 <50MB；历史峰值 659.5MB 仅为进程生命周期参考） |
...
```json
{
  "oversize": {
    "status": 413,
    "wall_sec": 2.14,
    "server_rss_before_mb": 89.1,
    "server_rss_after_mb": 91.7,
    "server_rss_peak_mb": 659.5,
    "server_rss_delta_mb": 2.6
  },
  "fat_60mb": {
    "zip_kb": 60.0,
    "e2e_ms_p50": 949.2,
